...
void AxisDrawing(double alpha, double beta, double gamma, Mat tvec, int index, double q2, double q3, vector<Point2f>& imPoints_repro, double L1, double L2, double L3);
...
int main(int argc, char** argv)
{
	...
	for (int k = 1; k <= NIMGS+1; k++) { //Bucle principal
		...
		if (im_validas == 3) { //Si las 3 imagenes son validas --> Triangulamos
			...
			AxisDrawing(alpha, beta, gamma, tvec, k, q2, q3, imPoints_reproyected,l1,l2,l3);
...
void AxisDrawing(double alpha, double beta, double gamma, Mat tvec, int index, double q2, double q3, vector<Point2f>& imPoints_repro, double L1, double L2, double L3)
{
	...
	imshow("Axis", src);
	sprintf_s(line, 99, "3gdl-%d.bmp", index);
	imwrite(line, src);
}
