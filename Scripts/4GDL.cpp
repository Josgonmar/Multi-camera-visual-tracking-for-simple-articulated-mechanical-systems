...
#define NIMGS 15
#define L1 0.320 //Tamaño de los eslabones, en metros
#define L2 0.245
#define L3 0.210
...
double cP2[3], cP3[3], cP4[3];
Mat src, threshold_img, distImage;

Eigen::MatrixXf Triangulation(vector<vector<Point2f>> imPoints, Eigen::MatrixXd c2Tc1, Eigen::MatrixXd c3Tc1);
void AxisDrawing(double alpha, double beta, double gamma, Mat tvec, int index, double q2, double q3, double q4, vector<Point2f>& imPoints_repro);
...
struct F7 {
	template <typename T>
	bool operator()(const T* const a, const T* const b, const T* const g, const T* const q2, const T* const q3, const T* const q4, T* residual) const {
		// f7 = (L2*sin(q2) + L3*sin(q2 + q3)*cos(q4))*cos(a)*cos(b)+(L3*sin(q4))*(cos(a)*sin(b)*sin(g)-sin(a)*cos(g))+(L1 + L2*cos(q2) + L3*cos(q2 + q3)*cos(q4))*(cos(a)*sin(b)*cos(g)+sin(a)*sin(g))+tx-cP4[0]
		residual[0] = (L2 * sin(q2[0]) + L3 * sin(q2[0] + q3[0]) * cos(q4[0])) * cos(a[0]) * cos(b[0]) + (L3 * sin(q4[0])) * (cos(a[0]) * sin(b[0]) * sin(g[0]) - sin(a[0]) * cos(g[0])) + (L1 + L2 * cos(q2[0]) + L3 * cos(q2[0] + q3[0]) * cos(q4[0])) * (cos(a[0]) * sin(b[0]) * cos(g[0]) + sin(a[0]) * sin(g[0])) + tx - cP4[0];
		return true;
	}
};
...
struct F8 {
	template <typename T>
	bool operator()(const T* const a, const T* const b, const T* const g, const T* const q2, const T* const q3,const T* const q4, T* residual) const {
		// f8 = (L2*sin(q2) + L3*sin(q2 + q3)*cos(q4))*sin(a)*cos(b)+(L3*sin(q4))*(sin(a)*sin(b)*sin(g)+cos(a)*cos(g))+(L1 + L2*cos(q2) + L3*cos(q2 + q3)*cos(q4))*(sin(a)*sin(b)*cos(g)-cos(a)*sin(g))+ty-cP4[1];
		residual[0] = (L2 * sin(q2[0]) + L3 * sin(q2[0] + q3[0]) * cos(q4[0])) * sin(a[0]) * cos(b[0]) + (L3 * sin(q4[0])) * (sin(a[0]) * sin(b[0]) * sin(g[0]) + cos(a[0]) * cos(g[0])) + (L1 + L2 * cos(q2[0]) + L3 * cos(q2[0] + q3[0]) * cos(q4[0])) * (sin(a[0]) * sin(b[0]) * cos(g[0]) - cos(a[0]) * sin(g[0])) + ty - cP4[1];
			return true;
	}
};
...
struct F9 {
	template <typename T>
	bool operator()(const T* const b, const T* const g, const T* const q2, const T* const q3, const T* const q4, T* residual) const {
		// f9 = (L2*sin(q2) + L3*sin(q2 + q3)*cos(q4))*(-sin(b))+(L3*sin(q4))*(cos(b)*sin(g))+(L1 + L2*cos(q2) + L3*cos(q2 + q3)*cos(q4))*(cos(b)*cos(g))+tz-cP4[2];
		residual[0] = (L2 * sin(q2[0]) + L3 * sin(q2[0] + q3[0]) * cos(q4[0])) * (-sin(b[0])) + (L3 * sin(q4[0])) * (cos(b[0]) * sin(g[0])) + (L1 + L2 * cos(q2[0]) + L3 * cos(q2[0] + q3[0]) * cos(q4[0])) * (cos(b[0]) * cos(g[0])) + tz - cP4[2];
		return true;
	}
};
...
int main(int argc, char** argv)
{
	...
	/*ofstream myfile;
	myfile.open("Total_time_4gdl.txt", ios::out | ios::trunc);
	myfile.close();
	myfile.open("nIterations_4gdl.txt", ios::out | ios::trunc);
	myfile.close();
	myfile.open("Full_report4gdl.txt", ios::out | ios::trunc);
	myfile.close();
	myfile.open("P3D_4gdl.txt", ios::out | ios::trunc);
	myfile.close();
	myfile.open("centers_resultsC1.txt", ios::out | ios::trunc);
	myfile.close();
	myfile.open("4gdl_repro.txt", ios::out | ios::trunc);
	myfile.close();*/

	// Matrices para la transformacion homogenea
	...
	for (int k = 1; k <= NIMGS+1; k++) { //Bucle principal
		...
		sprintf_s(path1, 99, "4gdl/2-%d.bmp", k);
		sprintf_s(path2, 99, "4gdl/1-%d.bmp", k);
		sprintf_s(path3, 99, "4gdl/3-%d.bmp", k);
		...
		int crop_right[] = { 170,170,170 };
		int im_validas = 0;
		vector<vector<Point2f>> imPoints(3);

		Eigen::Matrix4d c1Tw_promed;

		vector<Scalar> colores{ Scalar(255,0,0), Scalar(0,255,0), Scalar(0,0,255), Scalar(0,255,255) }; //BGR
		...
		for (i = 0; i < 3; i++) {
			...
			//Filtrado por circularidad, si se detectan mas contornos de los esperados
			if (contours_circ.size() > 4) {
			...
			if (ncomp > 0) {
				...
				if (ncomp == 4) {
					int ord_vec[] = { max_element(area.begin(),area.end()) - area.begin(), -1, -1, min_element(area.begin(),area.end()) - area.begin() }; //Ordenamos segun el area del marcador
					vector<float> area_copy(ncomp);
					area_copy = area;
					for (int v = 0; v < ncomp; v++) {
						if (area_copy[v] == area[ord_vec[0]]) area_copy[v] = 0;
						else if (area_copy[v] == area[ord_vec[3]]) area_copy[v] = 0;
					}

					for (int v = 0; v < ncomp; v++) {
						if (area_copy[max_element(area_copy.begin(), area_copy.end()) - area_copy.begin()] == area[v]) {
							ord_vec[1] = v;
						}
					}
					for (int v = 0; v < ncomp; v++) {
						if (area_copy[v] == area[ord_vec[1]]) area_copy[v] = 0;
					}
					...
					im_validas++;
					line(drawing, Point2f(pos_X[ord_vec[0]], pos_Y[ord_vec[0]]), Point2f(pos_X[ord_vec[1]], pos_Y[ord_vec[1]]), Scalar(200, 255, 0), 1);
					line(drawing, Point2f(pos_X[ord_vec[1]], pos_Y[ord_vec[1]]), Point2f(pos_X[ord_vec[2]], pos_Y[ord_vec[2]]), Scalar(200, 255, 0), 1);
					line(drawing, Point2f(pos_X[ord_vec[2]], pos_Y[ord_vec[2]]), Point2f(pos_X[ord_vec[3]], pos_Y[ord_vec[3]]), Scalar(200, 255, 0), 1);
				}
			}
		...
		if (im_validas == 3) { //Si las 3 imagenes son validas --> Triangulamos
			...
			problem.AddResidualBlock(new ceres::AutoDiffCostFunction<F7, 1, 1, 1, 1, 1, 1, 1>(new F7), NULL, &alpha, &beta, &gamma, &q2, &q3, &q4);
			problem.AddResidualBlock(new ceres::AutoDiffCostFunction<F8, 1, 1, 1, 1, 1, 1, 1>(new F8), NULL, &alpha, &beta, &gamma, &q2, &q3, &q4);
			problem.AddResidualBlock(new ceres::AutoDiffCostFunction<F9, 1, 1, 1, 1, 1, 1>(new F9), NULL, &beta, &gamma, &q2, &q3, &q4);

			//Limites inferiores y superiores de las variables
			...
			problem.SetParameterLowerBound(&q3, 0, -M_PI);
			problem.SetParameterLowerBound(&q4, 0, -M_PI);
			...
			problem.SetParameterUpperBound(&q3, 0, M_PI);
			problem.SetParameterUpperBound(&q4, 0, M_PI);
			...
			AxisDrawing(alpha, beta, gamma, tvec, k, q2, q3, q4, imPoints_reproyected);

			/*sprintf_s(aux, 99, "4gdl_repro.txt");
			...
			sprintf_s(aux, 99, "centers_resultsC1.txt");
			myfile.open(aux, ios::out | ios::app);
			for (int i = 0; i < 4; i++) {
				myfile << imPoints[0][i].x << "," << imPoints[0][i].y << endl;
			}
			myfile.close();

			sprintf_s(aux, 99, "Total_time_4gdl.txt");
			myfile.open(aux, ios::out | ios::app);
			myfile << summary.total_time_in_seconds << endl;
			myfile.close();

			sprintf_s(aux, 99, "nIterations_4gdl.txt");
			myfile.open(aux, ios::out | ios::app);
			myfile << summary.num_successful_steps + summary.num_unsuccessful_steps << endl;
			myfile.close();

			sprintf_s(aux, 99, "Full_report4gdl.txt");
			myfile.open(aux, ios::out | ios::app);
			myfile << alpha * 180 / M_PI << " " << beta * 180 / M_PI << " " << gamma * 180 / M_PI << " " << q2 * 180 / M_PI << " " << q3 * 180 / M_PI << " " << q4 * 180 / M_PI << endl;
			myfile.close();*/
...
Eigen::MatrixXf Triangulation(vector<vector<Point2f>> imPoints, Eigen::MatrixXd c2Tc1, Eigen::MatrixXd c3Tc1)
{
	...
	points3D = (points3D_c1c2.block(0,0,3,4) + points3D_c1c3.block(0,0,3,4)) / 2.0;

	sprintf_s(aux, 99, "P3D_4gdl.txt"); //Guardamos en un archivo para estudiar en MATLAB
	myfile.open(aux, ios::out | ios::app);
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 3; j++) {
...
void AxisDrawing(double alpha, double beta, double gamma, Mat tvec, int index, double q2, double q3, double q4, vector<Point2f>& imPoints_repro)
{
	...
	vector<Point3f> objectPoints_repro = { Point3f(0,0,0), Point3f(0,0,L1), Point3f(L2 * sin(q2),0,L1 + L2 * cos(q2)), Point3f(L2 * sin(q2) + L3 * sin(q2 + q3) * cos(q4),L3 * sin(q4),L1 + L2 * cos(q2) + L3 * cos(q2 + q3) * cos(q4)) };
	...
	char path[99];
	sprintf_s(path, 99, "4gdl/2-%d.bmp", index);
	Mat src = imread(path);
	...
	char line[99];
	sprintf_s(line, 99, "q2=%f, q3=%f, q4=%f", q2 * 180 / M_PI, q3 * 180 / M_PI, q4 * 180 / M_PI);
