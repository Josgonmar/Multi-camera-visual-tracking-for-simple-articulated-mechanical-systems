...
#define NIMGS 15
#define L1 0.32 //Dimensiones de los eslabones, en metros
#define L2 0.245
#define L3 0.210
...
void AxisDrawing(double alpha, double beta, double gamma, Mat tvec, int index, double q2, double q3, vector<Point2f>& imPoints_repro);
...
struct F7 {
	template <typename T>
	bool operator()(const T* const a, const T* const b, const T* const g, const T* const q2, const T* const q3, T* residual) const {
		// f7 = (L2*sin(q2)+L3*sin(q2+q3))*cos(a)*cos(b)+(L1+L3*cos(q2+q3)+L2*cos(q2))*(cos(a)*sin(b)*cos(g)+sin(a)*sin(g))+tx-cP4[0];
		residual[0] = (L2 * sin(q2[0]) + L3 * sin(q2[0] + q3[0])) * cos(a[0]) * cos(b[0]) + (L1 + L3 * cos(q2[0] + q3[0]) + L2 * cos(q2[0])) * (cos(a[0]) * sin(b[0]) * cos(g[0]) + sin(a[0]) * sin(g[0])) + tx - cP4[0];
		return true;
	}
};
...
struct F8 {
	template <typename T>
	bool operator()(const T* const a, const T* const b, const T* const g, const T* const q2, const T* const q3, T* residual) const {
		// f8 = (L2*sin(q2)+L3*sin(q2+q3))*sin(a)*cos(b)+(L1+L3*cos(q2+q3)+L2*cos(q2))*(sin(a)*sin(b)*cos(g)-cos(a)*sin(g))+ty-cP4[0];
		residual[0] = (L2 * sin(q2[0]) + L3 * sin(q2[0] + q3[0])) * sin(a[0]) * cos(b[0]) + (L1 + L3 * cos(q2[0] + q3[0]) + L2 * cos(q2[0])) * (sin(a[0]) * sin(b[0]) * cos(g[0]) - cos(a[0]) * sin(g[0])) + ty - cP4[1];
			return true;
	}
};
...
struct F9 {
	template <typename T>
	bool operator()(const T* const b, const T* const g, const T* const q2, const T* const q3, T* residual) const {
		// f9 = (L2*sin(q2)+L3*sin(q2+q3))*(-sin(b))+(L1+L3*cos(q2+q3)+L2*cos(q2))*cos(b)*cos(g)+tz-cP4[2];
		residual[0] = (L2 * sin(q2[0]) + L3 * sin(q2[0] + q3[0])) * (-sin(b[0])) + (L1 + L3 * cos(q2[0] + q3[0]) + L2 * cos(q2[0])) * cos(b[0]) * cos(g[0]) + tz - cP4[2];
		return true;
	}
};
...
int main(int argc, char** argv)
{
	...
	/*ofstream myfile;
	myfile.open("Total_time_3gdl.txt", ios::out | ios::trunc);
	myfile.close();
	myfile.open("nIterations_3gdl.txt", ios::out | ios::trunc);
	myfile.close();
	myfile.open("Full_report3gdl.txt", ios::out | ios::trunc);
	myfile.close();
	myfile.open("P3D_3gdl.txt", ios::out | ios::trunc);
	myfile.close();
	myfile.open("centers_resultsC1.txt", ios::out | ios::trunc);
	myfile.close();
	myfile.open("3gdl_repro.txt", ios::out | ios::trunc);
	myfile.close();*/

	// Matrices de transformacion, provenientes de la calibracion extrinseca
	...
	for (int k = 1; k <= NIMGS+1; k++) { //Bucle principal
		...
		sprintf_s(path1, 99, "3gdl/2-%d.bmp", k);
		sprintf_s(path2, 99, "3gdl/1-%d.bmp", k);
		sprintf_s(path3, 99, "3gdl/3-%d.bmp", k);
		...
		vector<Scalar> colores{ Scalar(255,0,0), Scalar(0,255,0), Scalar(0,0,255), Scalar(0,233,255) }; //BGR
		...
		for (i = 0; i < 3; i++) {
			...
			//Filtrado por circularidad, si detecta mas contornos de los esperados
		...
		if (im_validas == 3) { //Si las 3 imagenes son validas --> Triangulamos
			...
			problem.AddResidualBlock(new ceres::AutoDiffCostFunction<F7, 1, 1, 1, 1, 1, 1>(new F7), NULL, &alpha, &beta, &gamma, &q2, &q3);
			problem.AddResidualBlock(new ceres::AutoDiffCostFunction<F8, 1, 1, 1, 1, 1, 1>(new F8), NULL, &alpha, &beta, &gamma, &q2, &q3);
			problem.AddResidualBlock(new ceres::AutoDiffCostFunction<F9, 1, 1, 1, 1, 1>(new F9), NULL, &beta, &gamma, &q2, &q3);
			...
			AxisDrawing(alpha, beta, gamma, tvec, k, q2, q3, imPoints_reproyected);

			/*sprintf_s(aux, 99, "3gdl_repro.txt");
			...
			sprintf_s(aux, 99, "Total_time_3gdl.txt");
			myfile.open(aux, ios::out | ios::app);
			myfile << summary.total_time_in_seconds << endl;
			myfile.close();

			sprintf_s(aux, 99, "nIterations_3gdl.txt");
			myfile.open(aux, ios::out | ios::app);
			myfile << summary.num_successful_steps + summary.num_unsuccessful_steps << endl;
			myfile.close();

			sprintf_s(aux, 99, "Full_report3gdl.txt");
			myfile.open(aux, ios::out | ios::app);
			myfile << alpha * 180 / M_PI << " " << beta * 180 / M_PI << " " << gamma * 180 / M_PI << " " << q2 * 180 / M_PI << " " << q3 * 180 / M_PI << endl;
			myfile.close();*/
...
Eigen::MatrixXf Triangulation(vector<vector<Point2f>> imPoints, Eigen::MatrixXd c2Tc1, Eigen::MatrixXd c3Tc1)
{
	...
	sprintf_s(aux, 99, "P3D_3gdl.txt"); //Guardamos en un archivo para estudiar en MATLAB
	myfile.open(aux, ios::out | ios::app);
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 3; j++) {
...
void AxisDrawing(double alpha, double beta, double gamma, Mat tvec, int index, double q2, double q3, vector<Point2f> &imPoints_repro)
{
	...
	vector<Point3f> objectPoints_repro = { Point3f(0,0,0), Point3f(0,0,L1), Point3f(L2 * sin(q2),0,L1 + L2 * cos(q2)), Point3f(L2 * sin(q2) + L3 * sin(q2 + q3),0,L1 + L3 * cos(q2 + q3) + L2 * cos(q2)) };
	...
	char path[99];
	sprintf_s(path, 99, "3gdl/2-%d.bmp", index);
	Mat src = imread(path);
	...
	char line[99];
	sprintf_s(line, 99, "q2=%f, q3=%f", q2 * 180 / M_PI, q3 * 180 / M_PI);
